...
export async function ensureWhatsappIndexes() {
  ...
  await Promise.all([
    ...
    db.collection('whatsappAutomationJobs').createIndex({ tenantId: 1, messageId: 1 }, { unique: true }),
    db.collection('whatsappAutomationJobs').createIndex({ tenantId: 1, status: 1, _id: 1 }),
    db.collection('whatsappAutomationUsage').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }),
    db.collection('whatsappLeads').createIndex({ tenantId: 1, conversationId: 1 }, { unique: true }),
