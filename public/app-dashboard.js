...
let setupStep = 0
function renderSetupStep() { const current = setupSteps[setupStep]; document.querySelectorAll('#business-setup-form label').forEach(label => { const field=label.querySelector('input,textarea');label.hidden = field.id !== current.field;field.required=!label.hidden }); document.querySelector('#setup-step-count').textContent = `STEP ${setupStep + 1} OF ${setupSteps.length}`; document.querySelector('#setup-question').textContent = current.title; document.querySelector('#setup-helper').textContent = current.helper; document.querySelector('#setup-progress').style.width = `${((setupStep + 1) / setupSteps.length) * 100}%`; document.querySelector('#setup-back').hidden = setupStep === 0; document.querySelector('#setup-next').textContent = setupStep === setupSteps.length - 1 ? 'Save business setup →' : 'Continue →'; const field = document.querySelector(`#${current.field}`); field.focus() }
function renderVerificationStep() { const form = document.querySelector('#verification-profile-form'); const current = verificationSteps[verificationStep]; form.querySelectorAll('.verification-grid label').forEach(label => { const field = label.querySelector('[name]');if(field.dataset.required===undefined)field.dataset.required=field.required?'true':'false';label.hidden = !current.fields.includes(field.name);field.required=!label.hidden&&field.dataset.required==='true' }); document.querySelector('#verification-step-count').textContent = `STEP ${verificationStep + 1} OF ${verificationSteps.length}`; document.querySelector('#verification-question').textContent = current.title; document.querySelector('#verification-helper').textContent = current.helper; document.querySelector('#verification-progress').style.width = `${((verificationStep + 1) / verificationSteps.length) * 100}%`; document.querySelector('#verification-back').hidden = verificationStep === 0; document.querySelector('#verification-next').textContent = verificationStep === verificationSteps.length - 1 ? 'Save onboarding information →' : 'Continue →' }
...
loadVerification = async function () {
  ...
  document.querySelector('#profile-check')?.classList.toggle('complete', profileComplete)
  const connectionText = document.querySelector('#connection-readiness')
  const profileAction = document.querySelector('#complete-onboarding')
  if (profileAction) profileAction.textContent = 'Complete basic business profile →'
...
document.querySelector('#setup-next').addEventListener('click', async () => { const form=document.querySelector('#business-setup-form'),error=form.querySelector('#setup-error'),success=form.querySelector('#setup-success'),current=form.querySelector(`#${setupSteps[setupStep].field}`);error.textContent='';success.textContent='';if(!current.value.trim()){error.textContent='Complete this answer to continue.';current.focus();return}if(setupStep<setupSteps.length-1){setupStep++;renderSetupStep();return}const services=form.querySelector('#business-services').value.split(/[\n,]/).map(value=>value.trim()).filter(Boolean);const response=await fetch('/api/workspace/setup',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({businessDescription:form.querySelector('#business-description').value,services,hours:form.querySelector('#business-hours').value})});const data=await response.json();if(!response.ok){error.textContent=data.error||'We could not save your business setup.';return}workspace.workspaceSetup={businessDescription:form.querySelector('#business-description').value,services,hours:form.querySelector('#business-hours').value};updateChecklist();await loadVerification();success.textContent='Business setup saved. Your basic business profile is complete.'})
